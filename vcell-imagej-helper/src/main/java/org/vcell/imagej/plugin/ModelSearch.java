...
public class ModelSearch extends ContextCommand {
	...
	public static class VCellSelection {
		...
		public VCellSelection(String theCacheKey, String userid, String modelName, String appName, String simname,
				String varName, int timePointIndex) {
			super();
...
//			VCellHelper.VCellModelSearch vcms = new VCellHelper.VCellModelSearch(VCellHelper.ModelType.bm,"tutorial","Tutorial_MultiApp","3D pde","Simulation4",null,null);
			...
			VCellHelper.VCellModelSearch vcms = new VCellHelper.VCellModelSearch(VCellHelper.ModelType.bm, null, null,
					null, null, null, null);
			...
			try {
				...
				while (iterator.hasNext()) {
					...
					final String modelNameTime = modelName + " - "
			...
			JComboBox<String> jcbModelNames = new StyledComboBox<String>(
			...
			JComboBox<String> jcbAppNames = new StyledComboBox<String>(
			...
			JComboBox<String> jcbSimNames = new StyledComboBox<String>(mapAppsToSims
					.get(jcbModelNames.getSelectedItem() + " " + jcbAppNames.getSelectedItem()).toArray(new String[0]));
					...
					new Thread(new Runnable() {
						...
						public void run() {
							...
							String modelName = (jcbModelNames.getSelectedItem() == null ? null
									: mapModelNameTimeToActualModelname.get(jcbModelNames.getSelectedItem())
											.toString());
							...
							VCellHelper.VCellModelSearch vcms = new VCellHelper.VCellModelSearch(
									VCellHelper.ModelType.bm, userid, modelName, appName, simName, null, null);
							try {
								ArrayList<VCellModelSearchResults> vcmsr = vcellHelper
										.getSearchedModelSimCacheKey(false, vcms, null);
								if (vcmsr.size() == 0) {
								...
								SwingUtilities.invokeAndWait(new Runnable() {
									...
									public void run() {
										try {
											displayProgressBar(true, "Setting Var Times GUI...", "VCell Model Loader",
													100, uiService);
											...
											for (int i = 0; i < ijVarInfosHolder[0].getIjVarInfo().size(); i++) {
												jcbVars.insertItemAt(""
														+ ijVarInfosHolder[0].getIjVarInfo().get(i).getName() + ":"
														+ ijVarInfosHolder[0].getIjVarInfo().get(i).getDomain() + " ("
											...
											if (vcellModelsInput.getDefaultValue() != null) {// If user provided an
																								// inital value for
																								// VCellSelection var in
																								// VCellPlugin
												...
												for (int i = 0; i < jcbVars.getModel().getSize(); i++) {
													if (jcbVars.getModel().getElementAt(i)
															.startsWith(defaultValue.varName + ":")) {
											...
											uiService.showDialog(
													"VCellHelper.ModelType.bm,\"colreeze\",\"" + modelName + "\",\""
															+ appName + "\",\"" + simName + "\",null,null\n"
															+ e.getMessage(),
													"GUI setupfailed", MessageType.ERROR_MESSAGE);

										} finally {
											displayProgressBar(false, "Getting Vars and Times...", "VCell Model Loader",
													25, uiService);
										}
								...
								displayProgressBar(false, "Getting Vars and Times...", "VCell Model Loader", 25,
										uiService);
								// e.printStackTrace();
								uiService.showDialog(
										"VCellHelper.ModelType.bm,\"colreeze\",\"" + modelName + "\",\"" + appName
												+ "\",\"" + simName + "\",null,null\n" + e2.getMessage(),
										"Search failed", MessageType.ERROR_MESSAGE);
							}
						}
					}).start();
				}
			...
			if (vcellModelsInput.getDefaultValue() != null) {// If user provided an inital value for VCellSelection var
																// in VCellPlugin
				final VCellSelection defaultValue = vcellModelsInput.getDefaultValue();
			...
			displayProgressBar(false, "Creating GUI...", "VCell Model Loader", 100, uiService);
			int response = JOptionPane.showConfirmDialog(applicationFrame, jp, "Select User Model App Sim",
			...
			VCellSelection result = new VCellSelection(cacheKeyHolder[0], userid, modelName, appName, simName,
					ijVarInfosHolder[0].getIjVarInfo().get(jcbVars.getSelectedIndex()).getName(),
					jcbTimes.getSelectedIndex());
...
//			}
	...
	@Parameter
	private VCellSelection vcellSelection = new VCellSelection("-1", "colreeze", "Monkeyflower_pigmentation_v2",
			"Pattern_formation", "WT", "A", 50);
...
//    }
	...
	public void run() {
		try {
			...
			IJDataList tpd = vcellHelper.getTimePointData(vcellSelection.theCacheKey, var,
					VCellHelper.VARTYPE_POSTPROC.NotPostProcess, time, 0);
			...
			uiService.showDialog(
					"theCacheKey,var,VCellHelper.VARTYPE_POSTPROC.NotPostProcess,time,0\n" + e.getMessage(),
