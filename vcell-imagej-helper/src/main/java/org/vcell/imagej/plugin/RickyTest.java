		...
		if (Action.equals("Line Plot")) {
			try {
				...
				uiService.showDialog(
						"Click \"Tools\" in the top left corner of the VCell Application, then \"Start Fiji (ImageJ) service\"\n",
			...
			VCellHelper.VCellModelSearch vcms = new VCellHelper.VCellModelSearch(
					VCellHelper.ModelType.valueOf(modelType), vCellUser, vCellModel, application, simulation, null,
					null);
			try {
				...
				uiService.showDialog(modelType + ", " + vCellUser + ", " + vCellModel + ", " + application + ", "
			...
			try {
				timePointData = vcellHelper.getTimePointData(theCacheKey, variable, VARTYPE_POSTPROC.NotPostProcess,
						timePointIndexes, 0);
			} catch (Exception e) {
				uiService.showDialog(
						modelType + ", " + vCellUser + ", " + vCellModel + ", " + application + ", " + simulation + ", "
								+ variable + ", " + timePoint + "\n" + e.getMessage(),
						"Get Data failed", MessageType.ERROR_MESSAGE);
				return;
			}
			...
			for (int i = 0; i < dataIndexes.length; i++) {
				...
				// ijTimeSeriesJobResults.data[0/*index of "r"*/][i+1/*data, skip 0 because it
				// holds copy of times*/][0/*0 always because we had only 1 timePoint=22.0*/]
			}

			try {
				// LINE plot of data at 1 timePoint along defined indexes
				// Create JFreechart x,y axis arrays for plotting x=data indexes, y=dataPoint
				// values
			...
			try {
				...
				timeSeries = vcellHelper.getTimeSeries(new String[] { variable }, new int[] { dataIndexes[0] },
						times[0], 1, times[times.length - 1], false, false, 0, Integer.parseInt(theCacheKey));
			} catch (Exception e) {
...
//			chart.getXYPlot().getRangeAxis().setRange(xAxisRange);//X
			...
			String theCacheKey = null;
			VCellHelper.VCellModelSearch vcms = new VCellHelper.VCellModelSearch(VCellHelper.ModelType.bm, vCellUser,
					vCellModel, application, simulation, null, null);
			try {
				ArrayList<VCellModelSearchResults> vcmsr = vcellHelper.getSearchedModelSimCacheKey(false, vcms, null);
				if (vcmsr.size() == 0) {
					throw new Exception("No Results for search found");
			...
			try {
				...
				IJDataList tpd = vcellHelper.getTimePointData(theCacheKey, var,
