...
public class ModelLoad extends ContextCommand {
	...
	/*
	 * @Parameter private String imageName;
	 */
	...
	public void run() {
		...
		try {
			...
			// e.printStackTrace();
			uiService.showDialog(
					"VCellHelper.ModelType.bm,vCellUser,vCellModel,application,simulation,null,null\n" + e.getMessage(),
					"Search failed", MessageType.ERROR_MESSAGE);
			displayProgressBar(false, null, null);
		}
		...
		try {
			...
			IJDataList tpd = vcellHelper.getTimePointData(theCacheKey, var, VCellHelper.VARTYPE_POSTPROC.NotPostProcess,
					time, 0);
